    ...
    <section className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[10px] text-muted-foreground font-mono">
          CTX: {news.length} EVENTS
        </span>
      </div>

      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto pr-1 mb-3"
      >
        {messages.map((msg) => (
      ...
      <div className="glass-card p-2 flex items-center gap-2">
        <input
          ...
          disabled={isTyping}
          className="flex-1 bg-transparent text-[13px] text-foreground placeholder:text-muted-foreground outline-none px-3 py-2 font-sans disabled:opacity-50"
        />
        <Button
          size="icon"
          onClick={handleSend}
          disabled={!input.trim() || isTyping}
          className="h-8 w-8 bg-primary hover:bg-primary/80 text-primary-foreground shrink-0 glow-primary"
        >
